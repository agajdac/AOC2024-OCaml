...
type mixed = Bool of bool | Pair of int * int
...
let extract_numbers ?(ignore_dos = true) line =
  ...
  let results =
    List.map matches ~f:(fun group ->
        ...
        | Some _ ->
            Pair
              ( Int.of_string @@ Re.Group.get group 1,
                Int.of_string @@ Re.Group.get group 2 )
  ...
  @@ List.fold results ~init:(0, true) ~f:(fun (acc, is_enabled) group ->
         ...
         | Pair (x, y) ->
             ((acc + if is_enabled || ignore_dos then x * y else 0), is_enabled))
