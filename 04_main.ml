...
let is_matching array direction x y =
  ...
  let aChar = Char.equal array.(x + (2 * fst direction)).(y + (2 * snd direction)) 'A' in
  let sChar = Char.equal array.(x + (3 * fst direction)).(y + (3 * snd direction)) 'S' in
  mChar && aChar && sChar
...
let is_matching_1 array x y =
  let diagOne =
    (Char.equal array.(x + 1).(y + 1) 'S' && Char.equal array.(x - 1).(y - 1) 'M')
    || (Char.equal array.(x + 1).(y + 1) 'M' && Char.equal array.(x - 1).(y - 1) 'S')
  in
  let diagTwo =
    (Char.equal array.(x - 1).(y + 1) 'S' && Char.equal array.(x + 1).(y - 1) 'M')
    || (Char.equal array.(x - 1).(y + 1) 'M' && Char.equal array.(x + 1).(y - 1) 'S')
...
let part_2 array x y = if is_valid_1 array x y && is_matching_1 array x y then 1 else 0
